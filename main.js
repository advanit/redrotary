...
function initialize (cb) {
	...
	rrpCloudantDB.find({selector:{gameId:GAMEID}}, function(er, result) {
		  ...
		  for( var i =0 ; i<numOfSteps ; i++) {
		  	var clue = store[i];
			var stepId = store[i].stepId-1;
		  	espeak_tts_conversion(clue.message, "/opt/RRP/audio_files/"+stepId+".wav");
		  }
		  cb();
	});
}
	...
	if(step < numOfSteps) {
		...
		*/
		...
		for (i = 0; i < numOfSteps; i++){
			var temp = store[i];
			if(step === temp.stepId-1){
			   console.log("Found Clue: "+temp.stepId);
			   clue = temp;
			}
		}
...
eventEmitter
	.on("ring", function (argument) {
		console.log("Ring!!!!");
		omx.play('/opt/RRP/audio_files/ring.mp3');
		ringing = true;
	})
	.on("playClue", function (argument) {
		omx.stop();
		console.log("Clue play!!!!");
		console.log('/opt/RRP/audio_files/'+step+'.wav');
		omx.play('/opt/RRP/audio_files/'+step+'.wav');
		step ++;
		ringing = false;
		gameplay();
	});
